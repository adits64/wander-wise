...
const TripForm = ({tripData}) => {
  ...
  const onEdit = async (formData) => {
    ...
    try{
      const response = await api.patch(`/trips/${tripData._id}`, newData);
      console.log(response);
      ...
      if(response.status === 200){
        ...
        toast.error(response.data.message || "Some error occured while updating trip");
      }
    }catch(error){
  ...
  return (
    <form onSubmit={form.handleSubmit( tripData ? onEdit : onSubmit )} className="min-h-screen px-4">
      <Card className="w-full md:w-2/3 lg:w-1/2 mx-auto mt-40 mb-20">
        <CardHeader>
          <CardTitle>
